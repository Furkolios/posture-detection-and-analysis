...
/* --- CP3: classifier — output is always a valid enum member -------- */
/* In Phase 1 the classifier returned random states from the 3-state
 * enum; in Phase 2 it returns thresholded states from the 6-state
 * enum. The original intent was to verify the CONTRACT (output is a
 * valid enum value), so we widen the acceptance set accordingly. */

static void test_classifier(void) {
    printf("CP3 — classifier output validity\n");
    bool all_valid = true;
    int counts[POSTURE_STATE_COUNT] = {0};
    for (int i = 0; i < CLASSIFIER_TRIALS; ++i) {
        PostureState s = classifier_classify((float)i);
        if (s < 0 || s >= POSTURE_STATE_COUNT) {
            all_valid = false;
            break;
        }
        counts[s]++;
    }
    printf("    distribution over %d trials: "
           "GOOD=%d MILD=%d FULL=%d LEAN_FW=%d LEAN_BK=%d LATERAL=%d\n",
           CLASSIFIER_TRIALS,
           counts[POSTURE_GOOD], counts[POSTURE_MILD_SLOUCH],
           counts[POSTURE_FULL_SLOUCH], counts[POSTURE_LEAN_FORWARD],
           counts[POSTURE_LEAN_BACK], counts[POSTURE_LATERAL_TILT]);
